...
public class Program
{
            ...
            switch (choice) {
                ...
                case 7:
                    if (checkRaceCreated()) {
                        ...
                        try {
                            success = raceManagerPopulateAndStore.writeAthletesToFile(
                                    dataFile.dataFilePath, _race.getRacersList());
                        } catch (IOException e) {
                            e.printStackTrace();
                        }
                        if (success) {
                ...
                case 8:
                    if (checkRaceCreated()) {
                        ...
                        try {
                            _race.setRacersList(raceManagerPopulateAndStore.getAthletesFromFile(dataFile.dataFilePath));
                        } catch (IOException e) {
                            e.printStackTrace();
                        }
                    }
                ...
                case 9:
                    if (checkRaceCreated()) {
                        saveSystemToFile();
                        if (_race.getRacers().length == 0) {
                            System.out.println("This race has no Athletes competing. Saved it anyway.");
                        }
                        else {
                            System.out.println("Complete Race saved to system file.");
                        }
                    }
                    else {
                        System.out.println("There is currently no race taking place.");
                    }
                    break;
                case 10:
                    if (checkRaceCreated()) {
                        System.out.println("Will overwrite all data for this race from system file.");
                    }
                    else {
                        System.out.println("Will load an existing race from system file.");
                    }
                    loadSystemFromFile();
                    break;
                default:
    ...
    public static void saveSystemToFile() {
        try (FileOutputStream fos = new FileOutputStream(dataFile.dataObjectFilePath);
            ObjectOutputStream oos = new ObjectOutputStream(fos)) {
            oos.writeObject(_race);
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
    ...
    public static void loadSystemFromFile() {
        try (FileInputStream fin = new FileInputStream(dataFile.dataObjectFilePath);
            ObjectInputStream ois = new ObjectInputStream(fin)) {
            _race = (Race)ois.readObject();
